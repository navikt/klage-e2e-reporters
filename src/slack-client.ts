...
class SlackClient {
   ...
   *
   * That message is posted as the app itself, without the bot name and icon the rest of the report carries.
   * Either of those overrides makes the message a `bot_message`, and Slack drops the `file_ids` of a
   * `bot_message` without a word: `chat.update` answers `ok: true` and the message ends up with no files at all.
   */
  async uploadFiles(
    ...
    try {
      ...
      return new SlackMessageThread(this, updated);
    } catch (error) {
      ...
      return new SlackMessageThread(this, posted);
    }
  }
  ...
  /**
   * Rewrites a message that is already in the thread, keeping the files attached to it: Slack leaves them alone
   * unless `file_ids` says otherwise.
   *
   * A failure is reported to the caller and nowhere else. The message is still there saying what it said before,
   * so a quoted copy of the text it was going to say would be noise rather than a rescue.
   */
  async rewriteMessage(message: ChatPostMessageResponse | ChatUpdateResponse, newMessage: string, blocks?: AnyBlock[]) {
    return await this.applyUpdate(message, newMessage, blocks);
  }
  ...
  async updateMessage(
    ...
    try {
      return await this.applyUpdate(message, newMessage, blocks, color);
    } catch (error) {
  ...
  /** The reply is returned as a message of its own, so what was said can be rewritten later. */
...
export class SlackMessageThread {
  ...
  /** Rewrites what this message says, keeping the files attached to it. */
  rewrite = (newMessage: string, blocks?: AnyBlock[]) => this.app.rewriteMessage(this.message, newMessage, blocks);
