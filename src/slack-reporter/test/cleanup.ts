...
import { getErrorCode } from '@/slack-client';
import { BOT_NAME, MARKERS } from '@/slack-reporter/test/constants';
...
/**
 * Both the bot and every marker have to match: the bot id rules out everyone else's messages, and the markers
 * rule out anything but the fake runs, in case the token is ever shared with a real reporter.
 */
const isTestReport = (message: HistoryMessage): boolean =>
  message.bot_id !== undefined && message.bot_id === selfBotId && markerCount(message) === MARKERS.length;

/** The reporter names the trigger in the context line of the main message, and in its fallback text. */
const markerCount = ({ text, blocks, attachments }: HistoryMessage): number => {
  const rendered = JSON.stringify([text, blocks, attachments]);

  return MARKERS.filter((marker) => rendered.includes(marker)).length;
};

/** Partial matches, so messages the tests left behind are not lost silently. */
const logKept = (messages: HistoryMessage[]) => {
  const kept = messages.filter(
    (message) => !isTestReport(message) && (message.username === BOT_NAME || markerCount(message) > 0),
  );
