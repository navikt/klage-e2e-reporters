...
/**
 * Playwright titles are free text, and the report lists them in the running list, the warnings and the slow
 * tests. One that fills a message on its own is what those lists have to survive.
 */
const LONG_TITLE = `viser en advarsel når ${'saksbehandleren venter på et svar som aldri kommer '.repeat(40)}`.trim();
...
export const createFailedRun = (suite: Suite, directory: string): TestCase[] => [
  ...
  createTest(suite, {
    ...
    results: [
      createResult({
        duration: 44_000,
        steps: [createStep(['Vent på svar'], 'test.step', 41_000)],
        attachments: [
          {
            name: 'warningMessage',
            contentType: 'text/plain',
            body: Buffer.from('Backend svarte etter 41s. Forventet under 10s.'),
          },
        ],
      }),
    ],
...
/**
 * What the failed run should look like in Slack once it is over: every failing attempt with the media it left
 * behind, and the test that only failed on its first attempt rewritten to say so. The report script checks the
 * thread against this, because neither the files nor the rewrite show up in the calls the reporter makes.
 */
export const EXPECTED_REPORT = {
  files: [
    // sender inn klagen, which fails three times and reports each attempt with its own media
    ...['error.txt', 'screenshot.png', 'video.webm', 'trace.zip', 'app-state.json', 'stdout.txt'],
    ...['error.txt', 'screenshot.png', 'video.webm', 'trace.zip', 'app-state.json', 'stdout.txt'],
    ...['error.txt', 'screenshot.png', 'video.webm', 'trace.zip', 'app-state.json', 'stdout.txt'],
    // viser kvittering, which fails once and passes on the retry
    ...['error.txt', 'screenshot.png'],
    // laster ned PDF, whose trace is missing from disk
    'error.txt',
  ],
  says: ['Flaky: Klage > viser kvittering', 'Failed: Klage > sender inn klagen'],
  // The failing attempt of the flaky test was rewritten where it stood, rather than left behind or posted again.
  saysNoMore: ['Failed: Klage > viser kvittering'],
};
