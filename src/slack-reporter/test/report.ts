...
import { createConfig, createSuite, takeResults } from '@/slack-reporter/test/fake-playwright';
import {
  createAttachments,
  createFailedRun,
  createSuccessfulRun,
...
const main = async () => {
  ...
  try {
    ...
    console.info('Checking what the thread ended up saying.');
    await verifyReport(startedAt, EXPECTED_REPORT);
  } finally {
...
const reportFakeRun = async (create: (suite: Suite) => TestCase[], status: FullResult['status']) => {
  ...
  if (errorCount() > 0) {
    throw new Error(`The reporter logged ${errorCount()} errors while reporting the ${status} run.`);
  }
};
...
const playRun = async (reporter: SlackReporter, tests: TestCase[]) => {
...
const playTest = async (reporter: SlackReporter, testCase: TestCase) => {
  for (const result of takeResults(testCase)) {
    // Recorded before the attempt begins, the way Playwright does, so the outcome of the test follows the run.
