...
const createReport = (failedSteps: StepReport[], errors: TestError[], title = 'fails'): TestReport => {
  ...
  const test = createTest(createSuite(), { title, line: 12, results: [failedResult] });
...
describe('formatTestDetails', () => {
  test('includes the errors when the budget allows', () => {
    const elements = formatTestDetails(createReport(steps(3, 40), [error(5_000)]), undefined, []);
  ...
  test('keeps the message within the budget', () => {
    for (const length of [10, 200, 2_000, 5_000, 50_000]) {
      const elements = formatTestDetails(createReport(steps(3, 40), [error(length)]), undefined, []);
  ...
  test('leaves the errors out when the steps have spent the budget', () => {
    const elements = formatTestDetails(createReport(steps(20, 250), [error(5_000)]), undefined, []);
  ...
  test('notes the steps the budget left out', () => {
    const elements = formatTestDetails(createReport(steps(15, 250), []), undefined, []);
  ...
  test('leaves out the trace command when there is no trace to open', () => {
    const elements = formatTestDetails(createReport(steps(1, 20), [error(50)]), undefined, []);

    expect(toFallbackText(elements)).not.toContain('show-trace');
  });

  test('opens the trace the attempt left behind', () => {
    const elements = formatTestDetails(createReport(steps(1, 20), [error(50)]), 'trace-2.zip', []);
